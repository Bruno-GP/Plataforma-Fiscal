import type { DetailLevelButton, DetailMode, DetailModeOption } from '../types';

export type { DetailMode } from '../types';
...
export const getSearchPlaceholder = (isSped: boolean, detailMode: DetailMode) => {
...
export const getDetailHeading = (isSped: boolean, detailMode: DetailMode) => {
  if (isSped) {
    return detailMode === 'fiscal' ? 'Expansao fiscal em 2 niveis' : 'Expansao em 4 niveis para SPED';
  }

  return 'Expansao em 4 niveis';
};

export const getDetailDescription = (isSped: boolean) => {
  if (isSped) {
    return 'No SPED, o detalhamento usa a hierarquia fiscal existente para leitura por regiao ou por NCM.';
  }

  return 'Alterne entre leitura por nota ou por regiao e abra a hierarquia em camadas ate chegar aos produtos.';
};

export const getDetailScopeLabel = (isSped: boolean, detailMode: DetailMode) => {
  if (isSped) {
    return detailMode === 'fiscal' ? 'detalhamento fiscal' : 'detalhamento por regiao';
  }

  return detailMode === 'nota' ? 'detalhamento por nota' : 'detalhamento por regiao';
};

export const getDetailSummaryText = (params: {
  isSped: boolean;
  detailMode: DetailMode;
  filteredNotasCount: number;
  notasTotal: number;
  filteredSpedRowsCount: number;
}) => {
  const { isSped, detailMode, filteredNotasCount, notasTotal, filteredSpedRowsCount } = params;

  if (isSped) {
    return `Exibindo ${filteredSpedRowsCount} linhas agregadas da hierarquia fiscal.`;
  }

  return detailMode === 'nota'
    ? `Exibindo ${filteredNotasCount} de ${notasTotal} notas carregadas em blocos de 100.`
    : `Exibindo ${notasTotal} notas consolidadas na hierarquia regional.`;
};

export const getEmptyDetailMessage = (params: { isLoading: boolean; searchTerm: string }) => {
  const { isLoading, searchTerm } = params;

  if (isLoading) return 'Carregando detalhamento...';
  if (searchTerm.trim()) return 'Nenhum detalhamento encontrado para a pesquisa informada neste modo.';
  return 'Nenhum dado de venda encontrado para o periodo selecionado.';
};

export const getDetailModeOptions = (isSped: boolean): DetailModeOption[] | undefined =>
  (isSped
    ? [
        { key: 'regiao', title: 'Detalhamento por regiao', description: 'Estado > cidade > NCM > produto' },
        { key: 'fiscal', title: 'Detalhamento fiscal', description: 'NCM > produto' },
      ]
    : undefined);

type BuildLevelButtonsParams = {
  isSped: boolean;
  detailMode: DetailMode;
  allNotesOpen: boolean;
  allNoteClientsOpen: boolean;
  allNcmsOpen: boolean;
  allRegionStatesOpen: boolean;
  allRegionCitiesOpen: boolean;
  allRegionClientsOpen: boolean;
  allSpedRegionStatesOpen: boolean;
  allSpedRegionCitiesOpen: boolean;
  allSpedRegionNcmsOpen: boolean;
  allFiscalNcmsOpen: boolean;
  noteAccordionValues: string[];
  noteClientAccordionValues: string[];
  ncmAccordionValues: string[];
  regionStateAccordionValues: string[];
  regionCityAccordionValues: string[];
  regionClientAccordionValues: string[];
  spedRegionStateAccordionValues: string[];
  spedRegionCityAccordionValues: string[];
  spedRegionNcmAccordionValues: string[];
  fiscalNcmAccordionValues: string[];
  setOpenNoteValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenNoteClientValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenNcmValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenRegionStateValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenRegionCityValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenRegionClientValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenSpedRegionStateValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenSpedRegionCityValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenSpedRegionNcmValues: (values: string[] | ((current: string[]) => string[])) => void;
  setOpenFiscalNcmValues: (values: string[] | ((current: string[]) => string[])) => void;
};

export const buildLevelButtons = ({
  isSped,
  detailMode,
  allNotesOpen,
  allNoteClientsOpen,
...
}: BuildLevelButtonsParams): DetailLevelButton[] => {
  if (!isSped) {
    const toggleLevelOne = () => {
      if (detailMode === 'regiao') {
        if (allRegionStatesOpen) {
          setOpenRegionStateValues([]);
          return;
        }
    ...
    return [
      {
        key: 'nivel-fiscal-1',
        title: 'NCM',
        isOpen: allFiscalNcmsOpen,
        onClick: () => setOpenFiscalNcmValues(allFiscalNcmsOpen ? [] : fiscalNcmAccordionValues),
      },
      {
        key: 'nivel-fiscal-2',
        title: 'Produto',
        isOpen: allFiscalNcmsOpen,
        onClick: () => setOpenFiscalNcmValues(fiscalNcmAccordionValues),
      },
    ];
  }
  ...
  return [
    {
      key: 'nivel-sped-1',
      title: 'Estado',
      isOpen: allSpedRegionStatesOpen,
      onClick: () => setOpenSpedRegionStateValues(allSpedRegionStatesOpen ? [] : spedRegionStateAccordionValues),
    },
    {
      key: 'nivel-sped-2',
      title: 'Cidade',
      isOpen: allSpedRegionCitiesOpen,
      onClick: () => {
        if (allSpedRegionCitiesOpen) {
          setOpenSpedRegionCityValues([]);
          return;
        }
        setOpenSpedRegionStateValues(spedRegionStateAccordionValues);
        setOpenSpedRegionCityValues(spedRegionCityAccordionValues);
      },
    },
    {
      key: 'nivel-sped-3',
      title: 'NCM',
      isOpen: allSpedRegionNcmsOpen,
      onClick: () => {
        if (allSpedRegionNcmsOpen) {
          setOpenSpedRegionNcmValues([]);
          return;
        }
        setOpenSpedRegionStateValues(spedRegionStateAccordionValues);
        setOpenSpedRegionCityValues(spedRegionCityAccordionValues);
        setOpenSpedRegionNcmValues(spedRegionNcmAccordionValues);
      },
    },
    {
      key: 'nivel-sped-4',

  ...
  return (
    <Accordion type="multiple" value={openNoteValues} onValueChange={onOpenNoteValuesChange} className="w-full">
      {notas.map((nota) => {
        ...
        const supplierValue = `fornecedor-${nota.numero_nf}-${nota.data_emissao}`;
          ...
          <AccordionItem key={notaValue} value={notaValue} className="border-b border-slate-800/80">
            <AccordionTrigger className="px-6 py-5 hover:no-underline">
              <div className="grid w-full gap-3 text-left md:grid-cols-[1fr_1fr] md:items-center">
                ...
                <div>
                  <p className={hierarchyLabelClass}>Valor total da compra</p>
            ...
            <AccordionContent className="px-6 pb-6">
              ...
              >
                <AccordionItem
                  value={supplierValue}
                  className="rounded-2xl border border-slate-800 bg-slate-900/75 px-4"
                >
                    ...
                    <div className="grid w-full gap-3 pr-4 text-left md:grid-cols-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm font-medium text-slate-100">
                          {nota.emitente_cnpj || 'Fornecedor nao identificado'}
                        </p>
                      </div>
                      <div>
                        <p className={hierarchyLabelClass}>Operacao</p>
                        <p className="mt-1 text-sm text-slate-300">{nota.natureza_operacao || 'Nao informada'}</p>
                      </div>
                      <div>
                        <p className={hierarchyLabelClass}>Empresa destinataria</p>
                        <p className="mt-1 text-sm text-slate-300">{nota.destinatario_nome || 'Nao informada'}</p>
                      </div>
                      <div>
                        <p className={hierarchyLabelClass}>Modelo</p>
                        <p className="mt-1 text-sm text-slate-300">{nota.modelo || 'Nao informado'}</p>
                      </div>
                    </div>
                  </AccordionTrigger>

...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn("border-b border-slate-800/80 transition-colors data-[state=selected]:bg-slate-800/75 hover:bg-slate-800/45", className)}
      {...props}
    />
  ),
);
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-12 px-4 text-left align-middle font-semibold uppercase tracking-[0.12em] text-slate-400 [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />

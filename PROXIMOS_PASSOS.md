...
| # | O que fazer | Arquivo/Módulo | Complexidade | Dependências |
|---|---|---|---|---|
| 3 | ✅ **Resolvido (2026-08-13).** Fila completa dos 5 arquivos-alvo refatorada com extração de helpers privados, sem alterar contratos públicos, suite 229 passed/10 skipped em cada etapa: `nfe_consulta_service.py` (4 helpers: `_montar_rankings_compras`, `_montar_rankings_vendas`, `_montar_dados_dashboard_vendas`, `_montar_dados_hierarquia_fiscal`), `sped_consulta_service.py` (`_montar_dados_hierarquia_fiscal`), `sped_importacao_service.py` (commit `14af8a2`: `_carregar_sped_em_tabelas` quebrado em 6 métodos por tipo de registro SPED), `reforma_tributaria_sync_service.py` (commit `ecfb132`: `_inserir_tributos_reforma_xml_importados` quebrado em `_processar_xml_reforma_importado` + `_processar_item_reforma_xml`), `login_service.py` (commit `5983733`: `registrar` quebrado em `_upsert_empresa_registro`/`_garantir_email_disponivel`/`_inserir_login`, `_autenticar_sem_cache` quebrado em `_buscar_login_para_autenticacao`/`_validar_bloqueio_login`). Pendências residuais: `_normalizar_top_cidades` (linha 505 de `nfe_consulta_service.py`) segue como código morto, decisão em aberto; SQL embutido direto em `login_service.py` (sem repository dedicado) é débito arquitetural pré-existente, registrado mas não corrigido nesta fila. | `API/app/services/nfe/nfe_consulta_service.py`, `API/app/services/sped/sped_consulta_service.py`, `API/app/services/sped/sped_importacao_service.py`, `API/app/services/reforma_tributaria/reforma_tributaria_sync_service.py`, `API/app/services/nfe/auth/login_service.py` | Complexo (cada um) | Teste de caracterização antes de cada extração; preservar contratos HTTP listados em `docs/backend-debito-tecnico-fase-0.md` |
...
| 6 | ✅ **Resolvido (2026-08-13).** Decisão registrada: log JSON estruturado em stdout (`API/app/core/audit.py` + `JsonFormatter` em `API/app/core/logger.py`, já com sanitização de email/CNPJ/token) é suficiente como auditoria, **desde que produção capture o stdout via agregador de log externo** (ELK/Datadog/CloudWatch etc.). Não persiste nem expõe endpoint de consulta dentro da API. Premissa documentada em `docs/security.md`; revisitar se ambiente de produção não tiver agregador configurado ou se compliance exigir auditoria consultável na própria aplicação. | `API/app/core/audit.py`, `docs/security.md` | Médio | Nenhuma |
...
| 8 | ✅ **Resolvido (2026-08-19).** O front-end atual não mantém `ChatWidget` nem a rota "Atualizações" como código morto; o que ainda estava desativado no menu de Fiscal foi reativado em `Painel/src/components/layout/AppSidebar.tsx`, e a documentação de front-end foi alinhada com o estado real. | `Painel/src/components/layout/AppSidebar.tsx`, `docs/frontend.md` | Simples | Nenhuma |
...
| 10 | ✅ **Resolvido (2026-08-18).** Documentada explicitamente a exceção de catálogo global para `GET /api/reforma-tributaria/tributos` em `docs/security.md`, e o checklist backend agora pede classificação clara para rotas globais sem CNPJ na URL. | `docs/security.md`, `docs/backend-pr-checklist.md` | Simples | Nenhuma |

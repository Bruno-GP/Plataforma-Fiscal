...
| # | O que fazer | Arquivo/Módulo | Complexidade | Dependências |
|---|---|---|---|---|
| 3 | ✅ Em andamento. `nfe_consulta_service.py` recebeu 4 helpers privados (`_montar_rankings_compras`, `_montar_rankings_vendas`, `_montar_dados_dashboard_vendas`, `_montar_dados_hierarquia_fiscal`) e `sped_consulta_service.py` ganhou a extração de `_montar_dados_hierarquia_fiscal` (2026-08-12). `sped_importacao_service.py` (2026-08-13, commit `14af8a2`): `_carregar_sped_em_tabelas` (150 linhas, 6 responsabilidades) quebrado em 6 métodos privados por tipo de registro SPED (`0150`, `0200`, `C100`, `E100`, `E110`, `C170`); dict UF-por-código movido para constante de módulo. `reforma_tributaria_sync_service.py` (2026-08-13, commit `ecfb132`): `_inserir_tributos_reforma_xml_importados` (130 linhas, loop de 3 níveis) quebrado em `_processar_xml_reforma_importado` + `_processar_item_reforma_xml`. Nenhum contrato público alterado em nenhum dos três; suite 229 passed, 10 skipped. `_normalizar_top_cidades` (linha 505 de `nfe_consulta_service.py`) segue como código morto, decisão em aberto. Próximo (e último) alvo da fila: `login_service.py`. | `API/app/services/nfe/nfe_consulta_service.py`, `API/app/services/sped/sped_consulta_service.py`, `API/app/services/sped/sped_importacao_service.py`, `API/app/services/reforma_tributaria/reforma_tributaria_sync_service.py`, `API/app/services/nfe/auth/login_service.py` | Complexo (cada um) | Teste de caracterização antes de cada extração; preservar contratos HTTP listados em `docs/backend-debito-tecnico-fase-0.md` |
